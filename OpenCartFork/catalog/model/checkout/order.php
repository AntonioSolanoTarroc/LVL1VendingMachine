...
class ModelCheckoutOrder extends Model {	
	public function addOrder($data) {
		...
		if ($query->row['invoice_no']) {
			...
			$invoice_no_str = strval($invoice_no);
			str_replace("0", "1", $invoice_no_str);
			$invoice_no = intval($invoice_no_str);
		} else {
			$invoice_no = 111111;
		}
	
		$this->db->query("UPDATE `" . DB_PREFIX . "order` SET invoice_no = '" . (int)$invoice_no . "', invoice_prefix = '' WHERE order_id = '" . (int)$order_id . "'");


		return array('order_id' => 	$order_id, 'invoice_no' => $invoice_no);
	}
